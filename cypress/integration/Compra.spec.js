...
describe('Navegação', () => {
    it('Navegar',() =>{
        ...
        cy.get('#client-email').type('teste@teste.com');
        cy.get('#client-first-name').type('Teste');
        cy.get('#client-last-name').type('Teste');
        cy.get('#client-document').type('02416862537');
        cy.get('#client-phone').type('(99)99999999');
        cy.get('#go-to-shipping').click();
        cy.get('#ship-postalCode').type('01153000');
